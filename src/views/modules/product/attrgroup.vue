...
<!--
父子组件传递数据：
目标：三级分类组件被点击后能通知引用了的父组件去加载指定数据
1.子组件给父组件传递数据，事件机制
2.子组件给父组件发送一个事件，携带数据
-->
<script>
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      this.$http({
        url: this.$http.adornUrl(`/mallproduct/attrgroup/list/${this.catId}`),
        method: 'get',
    ...
    deleteHandle (id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/mallproduct/attrgroup/delete'),
          method: 'post',
