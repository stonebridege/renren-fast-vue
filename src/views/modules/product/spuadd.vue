...
export default {
  ...
  methods: {
    ...
    getMemberLevels() {
      this.$http({
        url: this.$http.adornUrl("/mallmember/memberlevel/list"),
        method: "get",
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/mallproduct/attr/sale/list/${this.spu.catalogId}`
          ),
          method: "get",
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(
            `/mallproduct/attrgroup/${this.spu.catalogId}/withattr`
          ),
          method: "get",
          params: this.$http.adornParams({})
        }).then(({ data }) => {
    ...
    submitSkus() {
      ...
      this.$confirm("将要提交商品数据，需要一小段时间，是否继续?", "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl("/mallproduct/spuinfo/save"),
            method: "post",
